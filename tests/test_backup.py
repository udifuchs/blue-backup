...
def test_source_folder(
    ...
    """Test source folder missing or empty."""
    toml_file = tmp_path / "blue.toml"
    (tmp_path / "backup-target").mkdir()
    with toml_file.open("w") as tfile:
        tfile.write(
    ...
    # Test local source missing:
    blue_backup.main(str(toml_file), "--first-time")
    captured = capsys.readouterr()
    assert captured.err == "    Source folder is missing.\n"

    # Test local source empty:
    (tmp_path / "backup-source").mkdir()
    blue_backup.main(str(toml_file))
    captured = capsys.readouterr()
    assert captured.err == "    Source folder is empty.\n"

    # Test local source success:
    (tmp_path / "backup-source" / "file-to-backup").touch()
    blue_backup.main(str(toml_file))
    captured = capsys.readouterr()
    assert captured.err == ""

    # Test remote source missing:
    ...
    blue_backup.main(str(toml_remote_file))
    captured = capsys.readouterr()
    assert captured.err == "    Source folder is missing.\n"

    # Test remote source empty:
    (tmp_path / "backup-remote-source").mkdir()
    blue_backup.main(str(toml_remote_file))
    captured = capsys.readouterr()
    assert captured.err == "    Source folder is empty.\n"

    # Test remote source success:
    (tmp_path / "backup-remote-source" / "file-to-backup").touch()
    blue_backup.main(str(toml_remote_file))
    captured = capsys.readouterr()
    assert captured.err == ""

    # Test remote source wrong hostname:
    ...
    blue_backup.main(str(toml_bad_remote_file))
    captured = capsys.readouterr()
    assert (
        captured.err ==
        "    Failed connecting to 127.0.0.256: [Errno -2] Name or service not known\n"
    )
